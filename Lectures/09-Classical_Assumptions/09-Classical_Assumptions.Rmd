...
library(pacman)
p_load(ggthemes, viridis, knitr, extrafont, tidyverse, magrittr, wooldridge, stargazer, latex2exp, parallel, broom, ggforce,xaringanthemer)
# Define colors
red_pink <- "#e64173"
turquoise <- "#20B2AA"
orange <- "#FFA500"
...
data %>% 
  ...
  theme_minimal()+ xlab("Years of Education") + ylab("log(Monthly Earnings)")
```
...
```{R, ex_good_exog, echo = F,fig.height = 5}
cond_good
```
---
class: white-slide
...
```{R, ex_bad_exog, echo = F,fig.height = 5}
cond_bad
```
...
```{R, gen dataset, include = F, cache = T,fig.height = 5}
...
```{R, scatter1, echo = F, fig.fullwidth = T}
...
library(pagedown)
pagedown::chrome_print("09-Classical_Assumptions.html", output = "09-Classical_Assumptions.pdf")
```
