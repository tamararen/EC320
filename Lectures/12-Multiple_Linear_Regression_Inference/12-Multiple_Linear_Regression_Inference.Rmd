...
library(pacman)
p_load(ggthemes, viridis, knitr, extrafont, tidyverse, magrittr, wooldridge, stargazer, latex2exp, parallel, broom, kableExtra, ggforce, furrr,xaringanthemer)
# Define colors
red_pink <- "#e64173"
turquoise <- "#20B2AA"
orange <- "#FFA500"
...
library(pagedown)
pagedown::chrome_print("12-Multiple_Linear_Regression_Inference.html", output = "12-Multiple_Linear_Regression_Inference.pdf")
```
...
Homework is composed of two parts: analytical and computation 

- Due to issues with the second computation homework, I will drop the lowest score between the second and third computation homework. 
- I will not be dropping the analytical portion. 
- You will see an update to the homework assignments where the homeworks are split btw analytical and computational. 
...
- One can prove that $s_u^2$ is an unbiased estimator of $\sigma^2$.
...
6. **Non-autocorrelation:** The values of error terms have independent distributions (*i.e.,* $E[u_i u_j]=0, \forall i \text{ s.t. } i \neq j$)
7. **Normality:** The population error term is normally distributed with mean zero and variance $\sigma^2$ (*i.e.,* $u \sim N(0,\sigma^2)$)
